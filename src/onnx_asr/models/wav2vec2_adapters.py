...
class LanguageNotFoundError(ValueError):
    ...
    def __init__(self, language: str, available: list[str]):
        ...
        super().__init__(f"Language '{language}' has no adapter pack. Available languages: {', '.join(available)}.")
...
class LanguageNotSpecifiedError(ValueError):
    ...
    def __init__(self, available: list[str]):
        ...
        super().__init__(
            ...
            f"in config.json. Available languages: {', '.join(available)}."
        )
...
class Wav2Vec2Adapters(_AsrWithCtcDecoding):
    ...
    def __init__(  # noqa: D107
        ...
        self._adapter_files = {path.stem: path for path in sorted(Path(model_files["adapters"]).glob("*.npz"))}
        self._vocab_files = {
            path.stem.removesuffix("_vocab"): path for path in sorted(Path(model_files["vocabs"]).glob("*.txt"))
        }
    ...
    def languages(self) -> list[str]:
        ...
        return sorted(self._adapter_files)
    ...
    def _resolve_language(self, language: str) -> str:
        ...
        for candidate in (language, language.replace("_", "-").split("-")[0]):
            if candidate in self._adapter_files:
                return candidate
            alias = self._aliases.get(candidate.lower())
            if alias in self._adapter_files:
    ...
    def _load_pack(self, language: str) -> _LanguagePack:
        ...
        with np.load(self._adapter_files[language]) as data:
        ...
        with self._vocab_files[language].open("rt", encoding="utf-8") as f:
